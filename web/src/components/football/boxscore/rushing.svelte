<script lang="ts">
  import type { FootballGame, Rushing, Team } from "$lib/football/footballgame";

  class TeamRushing {
    Team: Team;
    Stats: Rushing[];
    constructor(public team: Team, public stats: Rushing[]) {
      this.Team = team;
      this.Stats = stats.sort((a, b) => b.RushingYards - a.RushingYards);
    }
  }
  ...
  const teams = [new TeamRushing(game.AwayTeam, game.AwayRushing), new TeamRushing(game.HomeTeam, game.HomeRushing)];
</script>
  ...
  {#each teams as team}
    <div class="w-full">
      <div class="team-title mt-10">
        ...
        <div class="font-extrabold">{team.Team.City} Rushing</div>
      </div>
